 ...
 * 
 * 
 * The file contains the cross-validation experiments for traids with k=5 and tetrads with k=15.
 * 
 */

package edu.umd.cs.ps.fakhraei_tcbb2014;
...
def triad_similarity_dir = base_dir+'K5_0.5'+java.io.File.separator;
def tetrad_similarity_dir = base_dir+'K15_0.5'+java.io.File.separator;
...
for (int k=0;k<folds;k++)
{
	...
	for (int j=1;j<=folds;j++)
	{
		...
		{
			InserterUtils.loadDelimitedDataTruth(insertCVTrain, interactions_dir+'PSL_Positive_Fold_'+j+'.txt');
			InserterUtils.loadDelimitedDataTruth(insertCVTrain, interactions_dir+'PSL_Negetive_Fold_'+j+'.txt');
			InserterUtils.loadDelimitedDataTruth(insertWLTrain, interactions_dir+'PSL_Positive_Fold_'+j+'.txt');
			InserterUtils.loadDelimitedDataTruth(insertWLTrain, interactions_dir+'PSL_Negetive_Fold_'+j+'.txt');
		}
	}

	System.out.print ".";
	// Adding the weight learning held-out to the final training data
	InserterUtils.loadDelimitedDataTruth(insertCVTrain, interactions_dir+'PSL_Positive_Fold_'+WLHoldoutFold+'.txt');
	InserterUtils.loadDelimitedDataTruth(insertCVTrain, interactions_dir+'PSL_Negetive_Fold_'+WLHoldoutFold+'.txt');

	// Reading the weight learning held-out as the the labels
	InserterUtils.loadDelimitedDataTruth(insertWLLabels, interactions_dir+'PSL_Positive_Fold_'+WLHoldoutFold+'.txt');
	InserterUtils.loadDelimitedDataTruth(insertWLLabels, interactions_dir+'PSL_Negetive_Fold_'+WLHoldoutFold+'.txt');
	...
	InserterUtils.loadDelimitedDataTruth(insertWLLabels, interactions_dir+'PSL_Positive_Fold_'+CVHoldoutFold+'.txt');
	InserterUtils.loadDelimitedDataTruth(insertWLLabels, interactions_dir+'PSL_Negetive_Fold_'+CVHoldoutFold+'.txt');
	InserterUtils.loadDelimitedDataTruth(insertCVIgnored, interactions_dir+'PSL_Positive_Fold_'+CVHoldoutFold+'.txt');
	InserterUtils.loadDelimitedDataTruth(insertCVIgnored, interactions_dir+'PSL_Negetive_Fold_'+CVHoldoutFold+'.txt');

	// Reading the cross-validation held out as labels for the final model
	InserterUtils.loadDelimitedDataTruth(insertCVLabels, interactions_dir+'PSL_Positive_Fold_'+CVHoldoutFold+'.txt');
	InserterUtils.loadDelimitedDataTruth(insertCVLabels, interactions_dir+'PSL_Negetive_Fold_'+CVHoldoutFold+'.txt');
